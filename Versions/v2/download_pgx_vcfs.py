...
2. Filter to global (non-SAS) samples only
3. Extract only PGx positions
4. Split into per-sample single-sample VCFs
"""
...
def _run_docker_bcftools(args: list, timeout: int = 600) -> subprocess.CompletedProcess:
...
def download_and_extract(
    ...
    the target samples and PGx positions for one chromosome.
    """
    url = _vcf_url(chrom)
    ...
    print(f"    Running bcftools view (may take several minutes for large chromosomes)...")
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout)
        if result.returncode != 0:
            print(f"    ⚠ bcftools returned {result.returncode}: {result.stderr[:500]}")
            return False
        print(f"    ✓ Extracted → {output_vcf}")
        return True
...
def merge_chromosome_vcfs(chrom_vcfs: list, output: str) -> bool:
    ...
    input_args = [f"/data/{vcf}" for vcf in chrom_vcfs]
    data_mount = str(DATA_DIR).replace("\\", "/")
    cmd = [
        "docker", "run", "--rm",
        "-v", f"{data_mount}:/data",
...
def split_to_single_sample(merged_vcf: str) -> int:
    ...
    # Get the sample list from the VCF
    data_mount = str(DATA_DIR).replace("\\", "/")
    ...
    print(f"  Splitting samples into individual VCFs using bcftools +split...")
    cmd = [
        "docker", "run", "--rm",
        "-v", f"{data_mount}:/data",
        "-v", f"{per_sample_mount}:/out",
        "biocontainers/bcftools:v1.9-1-deb_cv1",
        "bcftools", "+split", f"/data/{merged_vcf}",
        "-O", "v",
        "-o", "/out"
    ]
    r = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
    if r.returncode != 0:
        print(f"  ⚠ Failed to split VCF: {r.stderr}")
        return 0
        
    # Count generated VCFs
    count = len(list(PER_SAMPLE_DIR.glob("*.vcf")))
...
def run() -> dict:
    ...
    if not chroms_file.exists():
        print("  ⚠ Run Step 1 first (fetch_global_samples.py)")
        return {}
    ...
    chrom_vcfs = []
    for chrom in chroms:
        out_name = f"global_pgx_{chrom}.vcf.gz"
        ...
        ok = download_and_extract(
            chrom=chrom,
            samples_file="global_samples.txt",
    ...
    merged = "global_pgx_merged.vcf.gz"
    ...
    # Sort and Index
    data_mount = str(DATA_DIR).replace("\\", "/")
    print(f"  Sorting the merged VCF...")
    sort_cmd = [
        "docker", "run", "--rm",
        "-v", f"{data_mount}:/data",
        "biocontainers/bcftools:v1.9-1-deb_cv1",
    ...
    sorted_path = DATA_DIR / f"sorted_{merged}"
    if sorted_path.exists():
        shutil.move(sorted_path, DATA_DIR / merged)
