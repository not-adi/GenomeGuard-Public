...
def _ensure_docker_image() -> bool:
    ...
    if not result.stdout.strip():
        print("  Pulling PharmCAT Docker image (this is a one-time download)...")
        pull = subprocess.run(
...
def parse_pharmcat_json(output_dir: Path, sample_id: str) -> List[dict]:
    ...
    # Extract gene calls from match results
    ...
    for call in gene_calls:
        ...
        # Get diplotype and phenotype from phenotype file
        diplotype_str = "*1/*1"
        phenotype = ""
        if pheno_files:
            try:
...
def run() -> dict:
    """Execute Step 4 (PharmCAT pipeline)."""
    print("\n═══ Step 4: Run PharmCAT on All Samples ═══\n")
    print("  ℹ This step can take 30–60+ minutes for ~600 samples.")
    print("  ℹ The agent will go idle during execution to save tokens.\n")
    ...
    for i, vcf_path in enumerate(vcf_files):
        ...
        ok = run_pharmcat_on_sample(vcf_path, sample_out_dir)
        if ok:
            rows = parse_pharmcat_json(sample_out_dir, sample_id)
            all_rows.extend(rows)
        else:
